...
// Email configuration
const NOTIFICATION_EMAIL = "pullijuha88@gmail.com";
...
document.getElementById('taskForm').addEventListener('submit', (e) => {
    ...
    const emailBody = `
        ...
        <p>${creator} has created a new task:</p>
        <p><strong>${title}</strong></p>
        <p>Date: ${dateKey}</p>
        <p>Time: ${startTime} - ${endTime}</p>
        ${userSettings[creator].requiresApproval ? '<p>This task requires approval.</p>' : '<p>This task does not require approval.</p>'}
    `;

    sendEmail(NOTIFICATION_EMAIL, "New Calendar Activity", emailBody);
...
function deleteTask(dateKey, taskId) {
    if (confirm('Are you sure you want to delete this task?')) {
        ...
        const emailBody = `
            <h2>Task Deleted</h2>
            <p>A task has been deleted from the calendar.</p>
            <p>Date: ${dateKey}</p>
        `;
                ...
                taskContent.onclick = (e) => {
                    ...
                    if (!task.accepted && currentUser !== task.creator) {
                        if (confirm('Do you want to accept this task?')) {
                            ...
                            const acceptanceEmailBody = `
                                <h2>Task Accepted</h2>
                                <p>${currentUser} has accepted the task:</p>
                                <p><strong>${task.title}</strong></p>
                                <p>Date: ${dateKey}</p>
                                <p>Time: ${task.startTime} - ${task.endTime}</p>
                            `;

                            sendEmail(NOTIFICATION_EMAIL, "New Calendar Activity", acceptanceEmailBody);
                            renderCalendar();
                            renderYearView();
                        }
                    }
